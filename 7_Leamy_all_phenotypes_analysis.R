...
# plot results
sos <- readRDS(paste0('Leamy_sos_', num_perm, '_perms.RDS'))
caos <- readRDS(paste0('Leamy_sovs_no_BVH_', num_perm, '_perms.RDS'))
sovs <- readRDS(paste0('Leamy_sovs_', num_perm, '_perms.RDS'))


width <- 9
plot(x = caos[['bw12d']],     y = sos[['bw12d']])      # no QTL
ggsave(filename = 'scan_cao_bw12d.pdf', height = 2, width = width)

plot(x = caos[['bw3wk']],     y = sos[['bw3wk']])      # no QTL
ggsave(filename = 'scan_cao_bw3wk.pdf', height = 2, width = width)

plot(x = caos[['bw6wk']],     y = sos[['bw6wk']])      # concordant, plus one new vQTL
ggsave(filename = 'scan_cao_bw6wk.pdf', height = 2, width = width)

plot(x = caos[['scfat12wk']], y = sos[['scfat12wk']])  # concordant (three big mQTL/mvQTL)
ggsave(filename = 'scan_cao_scfat12wk.pdf', height = 2, width = width)

plot(x = caos[['gfat12wk']],  y = sos[['gfat12wk']])   # concordant (three big mQTL/mvQTL)
ggsave(filename = 'scan_cao_gfat12wk.pdf', height = 2, width = width)



plot(x = sovs[['bw12d']],     y = sos[['bw12d']])      # no QTL
ggsave(filename = 'scan_dglm_bw12d.pdf', height = 2, width = width)

plot(x = sovs[['bw3wk']],     y = sos[['bw3wk']])      # new QTL on chr 11
ggsave(filename = 'scan_dglm_bw3wk.pdf', height = 2, width = width)

plot(x = sovs[['bw6wk']],     y = sos[['bw6wk']])      # concordant plus two new vQTL
ggsave(filename = 'scan_dglm_bw6wk.pdf', height = 2, width = width)

plot(x = sovs[['scfat12wk']], y = sos[['scfat12wk']])  # concordant plus more evidence on 11 and 13 and one undiscovery on chr 15
ggsave(filename = 'scan_dglm_scfat12wk.pdf', height = 2, width = width)

plot(x = sovs[['gfat12wk']],  y = sos[['gfat12wk']])   # concordant (three QTL)
ggsave(filename = 'scan_dglm_gfat12wk.pdf', height = 2, width = width)
