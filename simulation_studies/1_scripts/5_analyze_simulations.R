...
# address will's question
# wq <- r %>% filter(qtl == 'none', bvh == TRUE)
# table(wq$LM_locusperm_ps < 0.05, wq$DGLMm_locusperm_ps < 0.05)
...
ci_tibble <-  r2 %>%
  ...
  mutate(se = (ci_hi - ci_lo)/2,
         ideal_value = case_when(qtl == 'none' ~ 0.05,
                                 ...
                                 qtl == 'vqtl' ~ 0.655,
                                 qtl == 'mvqtl' ~ 0.742),
         ideal_value_se = case_when(qtl == 'none' ~ 0,
                                    qtl == 'mqtl' ~ 0.0089,
                                    qtl == 'vqtl' ~ 0.0094,
                                    qtl == 'mvqtl' ~ 0.0086))
...
ci_tibble %>%
  ...
  ggplot() +
  # geom_vline(mapping = aes(xintercept = ideal_value), color = 'gray', size = 0.5) +
  geom_rect(mapping = aes(xmin = ideal_value - ideal_value_se,
                          xmax = ideal_value + ideal_value_se,
                          ymin = -1,
                          ymax = 1),
            fill = 'lightgray') +
  geom_segment(mapping = aes(x = ci_lo, xend = ci_hi), y = 0, yend = 0) +
...
ci_tibble %>%
  ...
  theme(plot.margin = margin(t = 2, r = 2, b = 2, l = 2, unit = 'pt'),
        ...
        strip.text.x = element_text(size = 12, margin = margin(t = 2, r = 2, b = 4, l = 2, unit = 'pt')),
        panel.background = element_rect(fill = 'gray95')) +
  ggsave(filename = 'simulation_studies/4_images/mvqtl_cis.pdf', height = 3, width = 5)
